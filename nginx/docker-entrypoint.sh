#!/bin/sh
# Shebang: indica que este script se debe ejecutar con (Bourne shell).
...
# Comprobamos si NO existe la clave o NO existe el certificado. -f comprueba si un archivo existe y es un archivo r # [ ! -f "$KEY_FILE"] → verdadero si NO existe la clave. [ ! -f "$CRT_FILE" ] → verdadero si NO existe el certificado. # El operador '||' (OR lógico) hace que entremos en el 'if' si falta cualquiera de los dos.
if [ ! -f "$KEY_FILE" ] || [ ! -f "$CRT_FILE" ]; then
  ...
  # Desglose de opciones: # - 'req'
  # → genera una solicitud de certificado (CSR) o un certificado directamente.
  ...
  #           en entornos automatizados: Nginx podrá leer la clave sin pedir passphrase. -days 365 → el certificado será válido durante 365 días.
  ...
  # -out → fichero donde guardar el certificado
  # -subj
          #/C=ES
          #→ define los campos del sujeto del certificado sin pedirlos de forma interactiva: → Country (país), ES = España
          #/ST=Andalucia → State/Province (Andalucía)
          #/L=Castilleja de la Cuesta /0=TicketLogger → Organización (nombre de la empresa/proyecto) /OU=DAW /CN=localhost
          #→ Localidad (Castilleja de la Cuesta)
          #→ Unidad organizativa (por ejemplo, "Departamento DAW") → Common Name (FQDN para el que se emite el certificado). Aquí usamos 'localhost' porque es para desarrollo local.
openssl req -x509 -nodes -days 365 \
    -newkey rsa:2048 \
    -keyout "$KEY_FILE" \
    -out "$CRT_FILE" \
    -subj "/C=ES/ST=Andalucia/L=Castilleja de la Cuesta/0=TicketLogger/OU=DAW/CN=localhost"
else
  ...
  # así podemos mantener el mismo certificado entre reinicios de contenedor. echo "[nginx] Certificado ya existe, reutilizándolo."
...
# Nginx en foreground, que es lo que Docker espera para mantener el contenedor vivo. El uso de 'exec' es muy importante:
